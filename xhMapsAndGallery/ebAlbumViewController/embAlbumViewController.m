...
#pragma mark - Get Value Of Album Type
//Get the value of typeOfAlbum from root view controller
-(void)setTypeOfAlbum:(NSString *)typeAlbum
{
    NSLog(@"The type's value is: %@", typeAlbum);
    
    if ([typeAlbum isEqualToString:@"photos"])
    {
        typeOfAlbum = @"photos";
...
-(void)initSectionButtons
{
    for (int i = 0; i < self.sumOfSections; i++) {
        ...
        NSString *str_btnTitle = [NSString stringWithFormat:@"%d",(i+1)];
...
// Init segment control. The postion of it is on top of the collection view.
// Two options in segment control: 1.Album 2.Photos
-(void)initSegmentControl
{
    NSArray *itemArray = [NSArray arrayWithObjects:@"Album", @"Photos", nil];
    albumType = [[UISegmentedControl alloc] initWithItems:itemArray];
    albumType.frame = CGRectMake(415.0f, 67.0f, 194.0f, 28.0f);
    albumType.segmentedControlStyle = UISegmentedControlStylePlain;
    [albumType addTarget:self action:@selector(albumTypeIndexChanged) forControlEvents:UIControlEventValueChanged];
    if ([typeOfAlbum isEqualToString:@"photos"]) {
        albumType.selectedSegmentIndex = 1;
    }
    else
    {
        albumType.selectedSegmentIndex = 0;
    }
    [self.view addSubview:albumType];
}
//According to the index of segment control, change the value of typeOfAlbum to change the layout of collectionView.
-(void) albumTypeIndexChanged
{
    switch (self.albumType.selectedSegmentIndex) {
        case 0:
        {
            typeOfAlbum = @"album";
//            [collectionView reloadData];
